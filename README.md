...
Release, and pushes the updated `Formula/<name>.rb` straight here. For `octl`, that's
[`.github/workflows/release.yml`](https://github.com/gotry-io/Octopus/blob/main/.github/workflows/release.yml)
in [gotry-io/Octopus](https://github.com/gotry-io/Octopus) — triggered by pushing a `v*` tag — which rewrites
the `version` / `url` / `sha256` fields below and commits. It pushes here using a `TAP_PUSH_TOKEN` secret on the
source repo (a PAT with `contents:write` on this repo).
